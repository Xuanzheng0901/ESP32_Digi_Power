...
lv_indev_t *lvgl_port_add_encoder(const lvgl_port_encoder_cfg_t *encoder_cfg)
{
    ...
    if (encoder_cfg->encoder_a_b != NULL) {
        ...
        ESP_ERROR_CHECK(iot_knob_register_cb(encoder_ctx->knob_handle, KNOB_LEFT, knob_cb, encoder_ctx));
        ESP_ERROR_CHECK(iot_knob_register_cb(encoder_ctx->knob_handle, KNOB_RIGHT, knob_cb, encoder_ctx));
    }
...
static void knob_cb(void *arg, void *data)
{
    ...
    lvgl_port_encoder_ctx_t *ctx = (lvgl_port_encoder_ctx_t *)data;
